...
int main(int argc, char **argv)
{
	...
	seed = tv.tv_usec;
	build_model(&net, lr_inicial, use_cpu, seed, num_clases, num_capas, num_unidades, id_hebra);

	// Cargar todos los datos en x_train, y_train, x_val, y_val, x_test e y_test
	cargar_datos(directorio, fich_datosX, fich_datosY, flag_sets, &num_ejemplos, num_train, num_val, num_test,
		porc_train, porc_val, porc_test, num_clases, 1234, &x_train, &y_train, &x_val, &y_val, &x_test, &y_test);

	if (id_hebra == 0) {
		mostrarDatosProblema(flag_sets, num_ejemplos, num_train, num_val, num_test, porc_train, porc_val, porc_test,
			num_capas, num_unidades, max_epochs, batch_size, patience_es, lr_inicial, reducir_lr, patience_lr, lr_final);
	}

    // Entrenamiento con early stopping
	mejor_loss = entrenamiento(net, x_train, y_train, x_val, y_val, num_clases, max_epochs, batch_size,
	...
	if (id_hebra == id_mejor) {
		...
		evaluar_modelo(net, x_train, y_train, x_val, y_val, x_test, y_test, num_clases);
